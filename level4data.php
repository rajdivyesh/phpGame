...
if(isset($_POST['submit'])){
    ...
    if($_SESSION['user_lives']>=1){
        if($count == 6 AND ($user_ip!=$sorted_ans)){
            ...
            header("Location: level4.php?err=$numdiff");
            ...
            if($user_ip == $sorted_ans){
                ...
                $numdiff= "Your numbers have been correctly ordered in descending order";
                header("Location: level4.php?err=$numdiff");
            }
            else{
               $_SESSION['user_lives'] = $_SESSION['user_lives']-1;
                $numdiff= "Your numbers have not been correctly ordered in descending order";
                header("Location: level4.php?err=$numdiff");
            }
